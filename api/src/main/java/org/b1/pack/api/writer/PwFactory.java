...
public abstract class PwFactory {

    public static PwFactory newInstance(String format) {
        for (PackService packService : ServiceLoader.load(PackService.class)) {
            PwFactory factory = packService.getPwFactory(format);
            if (factory != null) return factory;
        }
        throw new PackException("Unsupported format: " + format);
    }

    public abstract PackWriter createPackWriter(PwProvider provider) throws IOException;
